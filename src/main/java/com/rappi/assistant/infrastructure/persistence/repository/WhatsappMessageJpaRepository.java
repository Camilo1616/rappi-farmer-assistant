...
public interface WhatsappMessageJpaRepository extends JpaRepository<WhatsappMessageEntity, Long> {
    ...
    /* Contactos intentados hoy (cualquier status) — para panel follow-up */
    @Query("SELECT DISTINCT m FROM WhatsappMessageEntity m " +
           "WHERE m.sentAt >= :start AND m.sentAt < :end " +
           "AND (:userId IS NULL OR m.user.id = :userId)")
    List<WhatsappMessageEntity> findContactsSentToday(@Param("start") LocalDateTime start,
                                                       @Param("end") LocalDateTime end,
                                                       @Param("userId") Long userId);

    /* Historial: últimos N días, con status por mensaje */
    @Query("SELECT m FROM WhatsappMessageEntity m LEFT JOIN FETCH m.user " +
