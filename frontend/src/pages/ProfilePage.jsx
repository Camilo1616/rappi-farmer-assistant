  ...
  return (
    <div className={styles.page}>
      ...
      <section className={styles.card}>
        ...
        {aiError && <div style={{ padding:'12px 16px', borderRadius:8, background:'rgba(239,68,68,0.08)', color:'#EF4444', fontSize:13, marginBottom:12 }}>{aiError}</div>}
        ...
        {aiRec && (
          <>
            <div style={{ padding:'14px 18px', borderRadius:10, background:'linear-gradient(135deg,rgba(124,58,237,0.08),rgba(109,40,217,0.04))', border:'1px solid rgba(124,58,237,0.18)', color:'var(--text-primary)', fontSize:14, lineHeight:1.6, marginBottom:18 }}>
              {aiRec.message}
            </div>
                  ...
                  <thead>
                    <tr style={{ borderBottom:'2px solid var(--border)' }}>
                      {['Tienda','Canal','Prioridad','Por qué','Acción hoy'].map(h => (
                        <th key={h} style={{ textAlign:'left', padding:'8px 10px', color:'var(--text-secondary)', fontWeight:700, fontSize:11, textTransform:'uppercase', letterSpacing:'0.4px', whiteSpace:'nowrap' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {aiRec.priorities.map((p, i) => (
                      <tr key={p.storeCode ?? i} style={{ borderBottom:'1px solid var(--border)', background: i%2===0 ? 'transparent' : 'var(--bg-input)' }}>
                        ...
                        <td style={{ padding:'9px 10px', color:'var(--text-secondary)', fontSize:12 }}>{p.channel ?? '—'}</td>
                        ...
                        <td style={{ padding:'9px 10px', color:'var(--text-secondary)', fontSize:12, maxWidth:220 }}>{p.reason}</td>
                        <td style={{ padding:'9px 10px', color:'var(--text-primary)', fontSize:12, maxWidth:200 }}>{p.action}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      <section className={styles.card}>
        ...
        <div style={{ height:320, overflowY:'auto', display:'flex', flexDirection:'column', gap:10, padding:'12px 8px', marginBottom:12, borderRadius:10, background:'var(--bg-input)', border:'1px solid var(--border)' }}>
          {chatHistory.length === 0 && (
            <div style={{ textAlign:'center', color:'var(--text-secondary)', fontSize:13, margin:'auto' }}>Empieza preguntando algo sobre tu cartera 👆</div>
          )}
          {chatHistory.map((m, i) => (
            <div key={i} style={{ display:'flex', justifyContent: m.role==='user' ? 'flex-end' : 'flex-start', padding:'0 6px' }}>
              <div style={{ maxWidth:'78%', padding:'10px 14px', borderRadius:12,
                background: m.role==='user' ? 'linear-gradient(135deg,#7C3AED,#6D28D9)' : 'var(--bg-card)',
                color: m.role==='user' ? '#fff' : 'var(--text-primary)', fontSize:13, lineHeight:1.6,
          ...
          {chatLoading && (
            <div style={{ padding:'0 6px' }}>
              <div style={{ display:'inline-block', padding:'10px 16px', borderRadius:12, background:'var(--bg-card)', border:'1px solid var(--border)', color:'var(--text-secondary)', fontSize:13 }}>⏳ Pensando...</div>
            </div>
          )}
          <div ref={chatEndRef} />
        </div>
        <div style={{ display:'flex', gap:8 }}>
          <input value={chatInput} onChange={e => setChatInput(e.target.value)}
            ...
            style={{ flex:1, padding:'10px 14px', borderRadius:8, border:'1.5px solid var(--border)', background:'var(--bg-input)', color:'var(--text-primary)', fontSize:13, outline:'none' }}
          />
          ...
          {chatHistory.length > 0 && (
            <button onClick={() => setChatHistory([])} style={{ padding:'10px 14px', borderRadius:8, border:'1.5px solid var(--border)', background:'transparent', color:'var(--text-secondary)', fontSize:12, cursor:'pointer' }}>Limpiar</button>
          )}
        </div>
      </section>
